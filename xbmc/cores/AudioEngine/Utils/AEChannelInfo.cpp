...
void CAEChannelInfo::ResolveChannels(const CAEChannelInfo& rhs)
{
  ...
  // mix back ceter if not available in destination layout
  if (srcHasBC && !dstHasBC)
  {
    if (dstHasRL && !newInfo.HasChannel(AE_CH_BL) && !newInfo.HasChannel(AE_CH_SL))
      newInfo += AE_CH_BL;
    if (dstHasRR && !newInfo.HasChannel(AE_CH_BR) && !newInfo.HasChannel(AE_CH_SR))
      newInfo += AE_CH_BR;
    if (dstHasSL && !newInfo.HasChannel(AE_CH_BL) && !newInfo.HasChannel(AE_CH_SL))
      newInfo += AE_CH_SL;
    if (dstHasSR && !newInfo.HasChannel(AE_CH_BR) && !newInfo.HasChannel(AE_CH_SR))
      newInfo += AE_CH_SR;
  }
